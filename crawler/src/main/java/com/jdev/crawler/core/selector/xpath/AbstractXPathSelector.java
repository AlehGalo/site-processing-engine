...
import com.jdev.crawler.core.selector.content.XPathEvaluator;
...
abstract class AbstractXPathSelector implements ISelector {
    ...
    protected List<ISelectorResult> selectFromNodeList(final String name, final String xPath)
            throws XPathSelectionException, XPathExpressionException {
        final List<ISelectorResult> resultList = new ArrayList<ISelectorResult>();
        final Object obj = new XPathEvaluator(xPath, node, XPathConstants.NODESET).evaluate();
    ...
    protected List<ISelectorResult> selectFromNode(final String name, final String xPath)
            throws XPathExpressionException {
        final List<ISelectorResult> resultList = new ArrayList<ISelectorResult>();
        final Object object = new XPathEvaluator(xPath, node, XPathConstants.NODE).evaluate();
    ...
    protected List<ISelectorResult> selectFromString(final String name, final String xPath)
            throws XPathExpressionException {
        final List<ISelectorResult> resultList = new ArrayList<ISelectorResult>();
        final Object object = new XPathEvaluator(xPath, node, XPathConstants.STRING).evaluate();
